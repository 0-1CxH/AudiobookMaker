...
class TestLLMRequestFunctionsIntegration:
    ...
    @pytest.mark.skipif(
        not os.environ.get('LLM_API_URL') or not os.environ.get('LLM_API_KEY') or not os.environ.get('LLM_MODEL'),
        reason="需要设置LLM_API_URL和LLM_API_KEY和LLM_MODEL环境变量"
    )
    def test_real_api_call_with_sdk(self):
        ...
        assert model is not None, "LLM_MODEL环境变量未设置"
